...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/projects/${id}`).then(r => r.json()).then(p => {
      setForm({
        name: p.name || '',
        slug: p.slug || '',
        tag: p.tag || '',
  ...
  return (
    <AdminFormShell title="Edit Project" backHref="/admin/projects" onSubmit={handleSubmit} saving={saving} error={error}>
      <AdminField label="Project Name">
        <AdminInput value={form.name} onChange={v => set('name', v)} placeholder="Project name" />
      </AdminField>
      <AdminField label="Slug">
        <AdminInput value={form.slug} onChange={v => set('slug', v)} placeholder="project-slug" mono />
      </AdminField>
      ...
      <AdminField label="Tech Stack (comma separated)">
        <AdminInput value={form.stack} onChange={v => set('stack', v)} placeholder="Node.js, TypeScript, PostgreSQL" />
      </AdminField>
